...
class ReSpeaker_Mic_Array_v2:
    ...
    def people_record(self, output_wav_filename, silence_threshold = 2):
        '''
        사람의 목소리가 감지되면 녹음을 시작하고, 2초 이상 목소리가 감지되지 않으면 녹음 중단하는 함수
        '''
        p = pyaudio.PyAudio()
        stream = p.open(
            rate = self.RESPEAKER_RATE,
        ...
        try:
            while True:
                ...
                if is_voice:
                    if not recoding:
                        print('Peopel voice. Starting record')
                        recoding = True
                        silence_start_time = None
                    frames.append(a.tostring())
                elif recoding:
...
if __name__ == '__main__':
    ...
    # extract people voice
    output_wav_filename = 'output.wav'
    mic.people_record(output_wav_filename)
